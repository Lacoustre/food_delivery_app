// Seeds the `meals` collection in Firestore from the restaurant's full menu.
//
// SETUP (you need to do this yourself — I can't generate credentials for you):
// 1. Firebase Console → Project Settings → Service Accounts → Generate new
//    private key. This downloads a JSON file.
// 2. Save it somewhere OUTSIDE this repo (never commit a service account key)
//    and set the path below, or point GOOGLE_APPLICATION_CREDENTIALS at it:
//      GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json node migrate_meals.js
// 3. Images: every item below has imageUrl: '' since photos aren't ready yet.
//    Re-run with imageUrl filled in later, or update via the admin panel —
//    this script uses each item's `name` as the doc ID, so re-running is
//    idempotent (it updates existing docs rather than duplicating them).
...
const meals = [
  ...
  // ── Vegetarian Menu (deliberately mirrors items above where the PDF
  //    re-lists them — kept as its own category per the source menu) ────
...
async function seedMeals() {
  ...
  for (const meal of meals) {
    // Use the meal name as the doc ID (slugified) so re-running this script
    // updates existing items instead of creating duplicates.
    const docId = meal.name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');

    const ref = db.collection('meals').doc(docId);
    batch.set(ref, {
      name: meal.name,
      price: meal.price,
  ...
  await batch.commit();
  console.log(`Seeded ${meals.length} meals into Firestore.`);
}
